...
const getBaixas = async (tipoMovimentacao) => {
    try {
        // const response = await api.get(`/baixa/${tipoMovimentacao}`);
        const response = {data:[
            {
...
const saveBaixa = async (tipoMovimentacao, obj) => {
    try {
        // const response = await api.get("/service");
        if(obj.id_movimentacao_mov){
            const response = await api.put(`/baixa/${tipoMovimentacao}/${obj.id_movimentacao_mov}`, obj);
            return response;
        }else{
            const response = await api.post(`/baixa/${tipoMovimentacao}`, obj);
            return response;
        }
    } catch (error) {
        console.error("Erro ao buscar:", error);
